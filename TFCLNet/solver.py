...
class Solver(object):
    ...
    def build_model(self):
        self.model = TFC(win_size=self.win_size, seq_size=self.seq_size, c_in=self.input_c, c_out=self.output_c, d_model=self.d_model, e_layers=self.e_layers, fr=self.fr, tr=self.tr, dev=self.device).to(self.device)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.num_epochs)):
            ...
            memory_usage = torch.cuda.memory_allocated(self.device) / (1024 ** 2)
            print(
                "Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} | Time: {4:.2f}s | Memory Usage: {5:.2f}MB".format(
                    epoch + 1, train_steps, train_loss, vali_loss, elapsed_time, memory_usage))
            # print(
